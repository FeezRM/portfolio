...
export function TimelineItem({ item, isLast }: { item: Experience; isLast?: boolean }) {
  return (
    <article className="relative grid gap-6 pb-10 pl-9 md:grid-cols-[0.8fr_1.2fr] md:gap-10 md:pl-12">
      <span className="absolute left-0 top-2 h-3 w-3 rounded-full border border-cyan/70 bg-bg shadow-[0_0_0_6px_rgba(34,211,238,0.08)]" />
      {!isLast ? (
        <span className="absolute bottom-0 left-[5px] top-7 w-px bg-gradient-to-b from-cyan/30 to-white/5" />
      ) : null}
