...
type Letter = {
  char: string;
  color: string;       // current hex — always hex, never rgb(...)
  targetColor: string; // target hex
...
const LetterGlitch = ({
  ...
  // color is always hex here; interpolation is computed at draw time
  const drawLetters = () => {
    ...
    letters.current.forEach((letter, index) => {
      ...
      let fillColor = letter.color;
      if (letter.colorProgress < 1) {
        const startRgb = hexToRgb(letter.color);
        const endRgb = hexToRgb(letter.targetColor);
        if (startRgb && endRgb) {
          fillColor = interpolateColor(startRgb, endRgb, letter.colorProgress);
        }
      }
  ...
  const updateLetters = () => {
    ...
    for (let i = 0; i < updateCount; i++) {
      ...
      if (!smooth) {
        ...
        // Start transition from current hex color
  ...
  const handleSmoothTransitions = () => {
    ...
    letters.current.forEach(letter => {
      if (letter.colorProgress < 1) {
        ...
        if (letter.colorProgress >= 1) {
          // Snap color to target hex so next transition starts from a valid hex
